...
let () =
  Gstreamer.init ();
  Printf.printf "%s\n%!" (version_string ());
  let bin = Pipeline.parse_launch "videotestsrc ! appsink max_buffers=2 drop=true caps=video/x-raw-rgb,width=320,height=240,framerate='(fraction)'10/1,bpp=24,depth=24 name=sink" in

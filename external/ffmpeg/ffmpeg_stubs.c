...
CAMLprim value caml_ffmpeg_dec_openfile(value fname)
{
  ...
  /* Open the file */
  assert(av_open_input_file(&ffd->av_format_ctx, String_val(fname), NULL, 0, NULL) == 0);
  ...
  while (av_read_frame(ffd->av_format_ctx, &packet) >= 0)
    {
      if(packet.stream_index == ffd->video_stream)
        {
          avcodec_decode_video(ffd->av_codec_ctx, ffd->av_frame, &frame_finished, packet.data, packet.size);
          if (frame_finished)
            {
...
CAMLprim value caml_ffmpeg_enc_openfile(value _fname, value _fr, value _width, value _height, value _bitrate)
{
  ...
  //ffe->video_stream = add_video_stream(ffe->format_ctx, ffe->output_format->video_codec);
  ...
  */
  ...
  ffe->convert_ctx = sws_getContext(c->width, c->height, PIX_FMT_RGBA, c->width, c->height, c->pix_fmt, SWS_BICUBIC, NULL, NULL, NULL);
...
CAMLprim value caml_ffmpeg_enc_write_frame(value _ffe, value _f)
{
  ...
  if (c->pix_fmt != PIX_FMT_RGBA)
    {
      ...
      sws_scale(ffe->convert_ctx, (const uint8_t * const*)ffe->tmpframe->data, ffe->tmpframe->linesize, 0, c->height, ffe->frame->data, ffe->frame->linesize);
    }
  else
  ...
  if (ffe->format_ctx->oformat->flags & AVFMT_RAWPICTURE)
    ...
    {
      /* encode the image */
      out_size = avcodec_encode_video(c, ffe->video_buffer, ffe->video_buffer_size, ffe->frame);
        ...
        if (out_size > 0)
          {
            ...
            pkt.pts= av_rescale_q(c->coded_frame->pts, c->time_base, ffe->video_stream->time_base);
